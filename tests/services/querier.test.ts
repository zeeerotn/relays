...
describe('querier', () => {
  ...
  it('basic table create', () => {
    ...
    table
      .create.name('users')
      .column.name('id').type('bigint').primaryKey().notNull()
      .column.name('wallet').type('money').notNull()
      .column.name('first').type('varchar', { length: 155 }).notNull()
      .column.name('last').type('varchar', { length: 255 }).notNull()
      .column.name('birthdate').type('date')
      .column.name('hits').type('bigserial').default(0)
